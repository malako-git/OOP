...
class MainWindow():
    ...
    def __init__(self, master):
        ...
        self.id = Entry(self.master)
        self.id.place(x=610,y=25,width=40,height=20)
        self.label1 = Label(self.master, text="Haus ID:",fg="black").place(x=530,y=25,width=70,height=20)

    def get_user_id(self):
        userinput = self.id.get()
        return userinput
    ...
    def delete(self):
        conn = sqlite3.connect("haeuser.db", timeout=2)  # db erstellen und connecten
        cur = conn.cursor() # cursor erstellen
        print("lösche eintrag and oid " + self.id.get())
        try:
...
class EditHausWindow():
    ...
    def __init__(self, master):
        ...
        obj1 = MainWindow()
        hausid = obj1.get_user_id()
        cur.execute("SELECT * FROM HaeuserOne WHERE oid = " + hausid)
